...
The above will use PAQ8PX v144 to compress a file called `My_file` which is in `C:\` into an archive named `My_file.paq8pxv144` which is stored at the same directory as the file, using compression level 9 and using the 'Adaptive learning rate' switch. It will then test the archive because we passed the `-t` argument.
...
optional arguments:
  ...
  -v VERSION, --version VERSION
                        Version of PAQ8PX to use. Example: 144. Default is 144
  -l LEVEL, --level LEVEL
...
From the paq8px_v144 executable, valid levels and switches are the following:
...
You can run `paq8px_v144.exe` for more information. Please note that the script only does compression using paq8px compression level and switches and also does testing by providing the -t argument. Nothing else is implemented in this script.
